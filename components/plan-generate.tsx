...
export default memo(function GeneratePlan({ userId }: GeneratePlanProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // تأكد من userId إن مش undefined
      if (!userId) {
      ...
      if (planErr || !planRow) {
        console.error("خطأ في إدراج الخطة:", planErr || "no data returned");
        toast.error(planErr?.message || "فشل في إنشاء الخطة");
        return;
      }
